...
fn main() {
    let path = concat!(env!("CARGO_MANIFEST_DIR"), "/../../../content-files/tmpls.3cn");
    ...
    for entry in cfl.chunks.iter().filter(|c| c.id.ctg == CTG_TMAP).take(5) {
        println!("TMAP {}: size={} packed={}", entry.id, entry.cb, entry.is_packed());

        // Access raw (possibly compressed) bytes directly
        let raw: &Vec<u8> = match cfl.chunk_data.get(&(entry.id.ctg, entry.id.cno)) {
            Some(r) => r,
            None => { println!("  not in chunk_data map"); err_count += 1; continue; }
        };
        ...
        if raw.len() >= 8 {
            ...
            println!("  codec header: fmt=0x{:08X} ('{}') cb_dst={}", fmt, fmt_str, cb_dst);
        }
        ...
        match cfl.get_chunk_data(entry.id.ctg, entry.id.cno) {
            Ok(data) => {
                ...
                match BrTmap::from_bytes(&data) {
                    Ok(t) => { println!("  parsed: {}x{} type={} row_bytes={}", t.width, t.height, t.pixel_type, t.row_bytes); ok_count += 1; }
                    Err(e) => { println!("  parse FAILED: {e}"); err_count += 1; }
                }
            }
            Err(e) => { println!("  decompress FAILED: {e}"); err_count += 1; }
        }
    }
