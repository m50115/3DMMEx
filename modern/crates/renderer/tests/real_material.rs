...
fn convert_all_bmtl_to_gpu() {
    ...
    for entry in cfl.chunks.iter().filter(|c| c.id.ctg == CTG_MTRL) {
        ...
        for &c in &gpu_mat.gpu.base_color {
            assert!((0.0..=1.0).contains(&c), "base_color component out of range: {c}");
        }
        // Alpha always 1.0
        assert_eq!(gpu_mat.gpu.base_color[3], 1.0);
        // Coefficients in [0, 1]
        assert!((0.0..=1.0).contains(&gpu_mat.gpu.ambient),  "ambient out of range");
        assert!((0.0..=1.0).contains(&gpu_mat.gpu.diffuse),  "diffuse out of range");
        assert!((0.0..=1.0).contains(&gpu_mat.gpu.specular), "specular out of range");
        // Power non-negative
