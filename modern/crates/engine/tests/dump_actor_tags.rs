    ...
    for path in &samples {
        ...
        for chunk in cfl.chunks.iter().filter(|c| c.id.ctg == CTG_ACTR) {
            let data = match cfl.get_chunk_data(chunk.id.ctg, chunk.id.cno) {
                Ok(d) => d, Err(e) => { println!("{name} ACTR:{} get_data err: {e:?}", chunk.id.cno); continue; }
            };
            if data.len() < 44 { println!("{name} ACTR:{} too short {}", chunk.id.cno, data.len()); continue; }
            let arr: [u8; 44] = data[..44].try_into().unwrap();
            let actf = match ActorOnFile::from_bytes(&arr) {
                Ok(a) => a, Err(e) => { println!("{name} ACTR:{} parse err: {e:?}", chunk.id.cno); continue; }
            };

            let tag = actf.tag_tmpl;
            let ctg_str: String = tag.ctg.to_be_bytes().iter()
                .map(|&b| if b.is_ascii_graphic() || b == b' ' { b as char } else { '.' })
                .collect();
            print!("{name} ACTR:{} sid={} ctg='{}' cno={} → ", chunk.id.cno, tag.sid, ctg_str, tag.cno);

            if tag.ctg != CTG_TMPL { println!("NOT TMPL, skip"); continue; }

            // Find TMPL in tmpls.3cn
            let tmpl = match tmpls.chunks.iter().find(|c| c.id.ctg == CTG_TMPL && c.id.cno == tag.cno) {
                Some(t) => t,
                None => { println!("TMPL:{} NOT FOUND in tmpls.3cn", tag.cno); continue; }
            };

            // Find BMDL children
            let bmdl_children: Vec<_> = tmpl.children.iter().filter(|c| c.id.ctg == CTG_BMDL).collect();
            if bmdl_children.is_empty() { println!("TMPL:{} has no BMDL children", tag.cno); continue; }
            ...
            match tmpls.get_chunk_data(first.id.ctg, first.id.cno) {
                ...
                Ok(data) => match Model::from_bytes(&data) {
                    ...
                    Ok(m) => println!("BMDL:{} verts={} faces={} valid={} non_empty={}",
                        first.id.cno, m.vertices.len(), m.faces.len(),
                        m.has_valid_faces(), !m.vertices.is_empty()),
                }
            }
        }
    }
