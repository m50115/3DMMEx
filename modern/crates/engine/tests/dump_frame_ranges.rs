...
fn dump_frame_ranges() {
    ...
    let scen_chunks: Vec<_> = cfl.chunks.iter().filter(|c| c.id.ctg == CTG_SCEN).collect();
    ...
    for (si, scen) in scen_chunks.iter().enumerate() {
        ...
        for actr in &actr_children {
            let data = match cfl.get_chunk_data(actr.id.ctg, actr.id.cno) {
                Ok(d) => d,
                Err(e) => {
                    println!("  ACTR:{} err: {e:?}", actr.id.cno);
                    continue;
                }
            };
            ...
            match ActorOnFile::from_bytes(&arr) {
                ...
                Ok(a) => {
                    ...
                    println!(
                        "  ACTR:{} arid={} nfrm_first={} nfrm_last={} has_range={}  cno={}",
                        actr.id.cno, a.arid, a.nfrm_first, a.nfrm_last, has_range, a.tag_tmpl.cno
                    );
                }
            }
        }
